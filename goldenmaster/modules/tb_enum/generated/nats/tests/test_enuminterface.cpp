...
TEST_CASE("Nats  tb.enum EnumInterface tests")
{
    ...
    serviceEnumInterface->_subscribeForIsReady([&is_serviceReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
    ...
    if (serviceEnumInterface->_is_ready())
    {
        is_serviceReady = true;
    }
    lock.lock();
    ...
    clientEnumInterface->_subscribeForIsReady([&is_clientReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
                is_clientReady = true;
    ...
    if (clientEnumInterface->_is_ready())
    {
        is_clientReady = true;
    }
    lock.lock();
