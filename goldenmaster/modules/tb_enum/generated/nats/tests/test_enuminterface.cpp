...
TEST_CASE("Nats  tb.enum EnumInterface tests")
{
    ...
    SECTION("Test emit sig0")
    {
        ...
        [&m_wait, &issig0Emitted](Enum0Enum param0)
        {
    ...
    SECTION("Test emit sig1")
    {
        ...
        [&m_wait, &issig1Emitted](Enum1Enum param1)
        {
    ...
    SECTION("Test emit sig2")
    {
        ...
        [&m_wait, &issig2Emitted](Enum2Enum param2)
        {
    ...
    SECTION("Test emit sig3")
    {
        ...
        [&m_wait, &issig3Emitted](Enum3Enum param3)
        {
    ...
    {
        [[maybe_unused]] auto result = clientEnumInterface->func0(Enum0Enum::value0);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func0 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientEnumInterface->func0Async(Enum0Enum::value0);
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == Enum0Enum::value0);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func1")
    {
        [[maybe_unused]] auto result = clientEnumInterface->func1(Enum1Enum::value1);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func1 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientEnumInterface->func1Async(Enum1Enum::value1);
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == Enum1Enum::value1);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func2")
    {
        [[maybe_unused]] auto result = clientEnumInterface->func2(Enum2Enum::value2);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func2 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientEnumInterface->func2Async(Enum2Enum::value2);
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == Enum2Enum::value2);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func3")
    {
        [[maybe_unused]] auto result = clientEnumInterface->func3(Enum3Enum::value3);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func3 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientEnumInterface->func3Async(Enum3Enum::value3);
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == Enum3Enum::value3);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
