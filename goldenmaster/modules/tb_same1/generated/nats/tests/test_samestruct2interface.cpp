...
TEST_CASE("Nats  tb.same1 SameStruct2Interface tests")
{
    ...
    SECTION("Test emit sig2")
    {
        ...
        auto local_param1_struct = Struct1();
        TbSame1::fillTestStruct1(local_param1_struct);
        auto local_param2_struct = Struct2();
        TbSame1::fillTestStruct2(local_param2_struct);

        clientSameStruct2Interface->_getPublisher().subscribeToSig2(
        [&m_wait, &issig2Emitted, &local_param1_struct, &local_param2_struct](const Struct1& param1, const Struct2& param2)
        {
            REQUIRE(param1 ==local_param1_struct);
    ...
    {
        [[maybe_unused]] auto result = clientSameStruct2Interface->func1(Struct1());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func1 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientSameStruct2Interface->func1Async(Struct1());
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == Struct1()); 
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func2")
    {
        [[maybe_unused]] auto result = clientSameStruct2Interface->func2(Struct1(), Struct2());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func2 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientSameStruct2Interface->func2Async(Struct1(), Struct2());
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
