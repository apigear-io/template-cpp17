...
void SameStruct1InterfaceClient::init()
{
    if (m_initialized.exchange(true)) {
        AG_LOG_WARNING("init() called more than once on " "SameStruct1InterfaceClient" ", ignoring");
        return;
    }
    std::weak_ptr<SameStruct1InterfaceClient> weak_self = shared_from_this();
...
void SameStruct1InterfaceClient::onConnected()
{
    ...
    subscribeTopic("tb.same1.SameStruct1Interface.service.available",[this](const auto& value){ handleAvailable(value); });
    const std::string initTopic =  "tb.same1.SameStruct1Interface.init.resp." + std::to_string(clientId);
