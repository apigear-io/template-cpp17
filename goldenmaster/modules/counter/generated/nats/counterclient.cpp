...
std::future<std::list<Eigen::Vector3f>> CounterClient::incrementArrayAsync(const std::list<Eigen::Vector3f>& vec, std::function<void(std::list<Eigen::Vector3f>)> user_callback)
{
    if(m_client == nullptr) {
        throw std::runtime_error("Client is not initialized");
    }
    ...
    return std::async(std::launch::async, [this, user_callback,vec]()
    {
        ...
        auto callback = [&resultPromise, user_callback](const auto& result)
        {
            if (result.empty())
            {
                ...
                if (user_callback)
                {
                    user_callback(std::list<Eigen::Vector3f>());
                }
                return;
            }
            nlohmann::json field = nlohmann::json::parse(result);
...
std::future<std::list<Test::CustomTypes::Vector3D>> CounterClient::decrementArrayAsync(const std::list<Test::CustomTypes::Vector3D>& vec, std::function<void(std::list<Test::CustomTypes::Vector3D>)> user_callback)
{
    if(m_client == nullptr) {
        throw std::runtime_error("Client is not initialized");
    }
    ...
    return std::async(std::launch::async, [this, user_callback,vec]()
    {
        ...
        auto callback = [&resultPromise, user_callback](const auto& result)
        {
            if (result.empty())
            {
                ...
                if (user_callback)
                {
                    user_callback(std::list<Test::CustomTypes::Vector3D>());
                }
                return;
            }
            nlohmann::json field = nlohmann::json::parse(result);
