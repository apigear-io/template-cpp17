...
void StructInterfaceService::onConnected()
{
    ...
    m_service->publish("testbed1.StructInterface.prop.propBool", nlohmann::json(m_impl->getPropBool()).dump());
    m_service->publish("testbed1.StructInterface.prop.propInt", nlohmann::json(m_impl->getPropInt()).dump());
    m_service->publish("testbed1.StructInterface.prop.propFloat", nlohmann::json(m_impl->getPropFloat()).dump());
    m_service->publish("testbed1.StructInterface.prop.propString", nlohmann::json(m_impl->getPropString()).dump());
}
