...
TEST_CASE("Nats  testbed1 StructInterface tests")
{
    ...
    SECTION("Test emit sigBool")
    {
        ...
        auto local_param_bool_struct = StructBool();
        Testbed1::fillTestStructBool(local_param_bool_struct);

        clientStructInterface->_getPublisher().subscribeToSigBool(
        [&m_wait, &issigBoolEmitted, &local_param_bool_struct](const StructBool& paramBool)
        {
    ...
    SECTION("Test emit sigInt")
    {
        ...
        auto local_param_int_struct = StructInt();
        Testbed1::fillTestStructInt(local_param_int_struct);

        clientStructInterface->_getPublisher().subscribeToSigInt(
        [&m_wait, &issigIntEmitted, &local_param_int_struct](const StructInt& paramInt)
        {
    ...
    SECTION("Test emit sigFloat")
    {
        ...
        auto local_param_float_struct = StructFloat();
        Testbed1::fillTestStructFloat(local_param_float_struct);

        clientStructInterface->_getPublisher().subscribeToSigFloat(
        [&m_wait, &issigFloatEmitted, &local_param_float_struct](const StructFloat& paramFloat)
        {
    ...
    SECTION("Test emit sigString")
    {
        ...
        auto local_param_string_struct = StructString();
        Testbed1::fillTestStructString(local_param_string_struct);

        clientStructInterface->_getPublisher().subscribeToSigString(
        [&m_wait, &issigStringEmitted, &local_param_string_struct](const StructString& paramString)
        {
    ...
    {
        [[maybe_unused]] auto result = clientStructInterface->funcBool(StructBool());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method funcBool async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientStructInterface->funcBoolAsync(StructBool());
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == StructBool()); 
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method funcInt")
    {
        [[maybe_unused]] auto result = clientStructInterface->funcInt(StructInt());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method funcInt async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientStructInterface->funcIntAsync(StructInt());
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
    ...
    {
        [[maybe_unused]] auto result = clientStructInterface->funcFloat(StructFloat());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method funcFloat async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientStructInterface->funcFloatAsync(StructFloat());
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == StructFloat()); 
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method funcString")
    {
        [[maybe_unused]] auto result = clientStructInterface->funcString(StructString());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method funcString async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientStructInterface->funcStringAsync(StructString());
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
        auto return_value = resultFuture.get();
        REQUIRE(return_value == StructString()); 
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
