...
TEST_CASE("Nats  testbed1 StructInterface tests")
{
    ...
    serviceStructInterface->_subscribeForIsReady([&is_serviceReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
    ...
    if (serviceStructInterface->_is_ready())
    {
        is_serviceReady = true;
    }
    lock.lock();
    ...
    clientStructInterface->_subscribeForIsReady([&is_clientReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
                is_clientReady = true;
    ...
    if (clientStructInterface->_is_ready())
    {
        is_clientReady = true;
    }
    lock.lock();
