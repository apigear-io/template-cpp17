...
TEST_CASE("olink  tb.simple SimpleInterface tests")
{
    ...
    auto clientSimpleInterface = std::make_shared<Test::TbSimple::olink::SimpleInterfaceClient>();
    ...
    auto serviceSimpleInterface = std::make_shared<Test::TbSimple::olink::SimpleInterfaceService>(implSimpleInterface, remote_registry);
    ...
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientSimpleInterface->funcNoReturnValueAsync(false,[&finished, &m_wait](){finished = true; m_wait.notify_all(); /* YOU CAN CHECK EFFECTS OF YOUR METHOD HERE */ });
