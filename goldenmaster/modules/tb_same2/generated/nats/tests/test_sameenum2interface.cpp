...
TEST_CASE("Nats  tb.same2 SameEnum2Interface tests")
{
    ...
    SECTION("Test emit sig2")
    {
        ...
        [&m_wait, &issig2Emitted](Enum1Enum param1, Enum2Enum param2)
        {
            REQUIRE(param1 == TbSame2::Enum1Enum::value2);
    ...
    {
        [[maybe_unused]] auto result = clientSameEnum2Interface->func1(Enum1Enum::value1);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func1 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientSameEnum2Interface->func1Async(Enum1Enum::value1);
    ...
    {
        [[maybe_unused]] auto result = clientSameEnum2Interface->func2(Enum1Enum::value1, Enum2Enum::value1);
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func2 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientSameEnum2Interface->func2Async(Enum1Enum::value1, Enum2Enum::value1);
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
