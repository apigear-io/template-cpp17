...
TEST_CASE("Nats  tb.same2 SameStruct1Interface tests")
{
    ...
    serviceSameStruct1Interface->_subscribeForIsReady([&is_serviceReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
    ...
    if (serviceSameStruct1Interface->_is_ready())
    {
        is_serviceReady = true;
    }
    lock.lock();
    ...
    clientSameStruct1Interface->_subscribeForIsReady([&is_clientReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
                is_clientReady = true;
    ...
    if (clientSameStruct1Interface->_is_ready())
    {
        is_clientReady = true;
    }
    lock.lock();
