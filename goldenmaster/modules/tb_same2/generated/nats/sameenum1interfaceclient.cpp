...
void SameEnum1InterfaceClient::onConnected()
{
    ...
    subscribeTopic("tb.same2.SameEnum1Interface.service.available",[this](const auto& value){ handleAvailable(value); });
    const std::string initTopic =  "tb.same2.SameEnum1Interface.init.resp." + std::to_string(clientId);
    subscribeTopic(initTopic,[this](const auto& value){ handleInit(value); });
    const std::string topic_prop1 =  "tb.same2.SameEnum1Interface.prop.prop1";
    subscribeTopic(topic_prop1, [this](const auto& value){ setProp1Local(_to_Prop1(value)); });
    const std::string topic_sig1 = "tb.same2.SameEnum1Interface.sig.sig1";
