...
std::shared_ptr<SameEnum1InterfaceClient> SameEnum1InterfaceClient::create(std::shared_ptr<ApiGear::MQTT::Client> client)
{
    return std::make_shared<SameEnum1InterfaceClient>(client);
}
...
void SameEnum1InterfaceClient::setProp1Local(const std::string& args)
{
    try {
        ...
        AG_LOG_ERROR("SameEnum1InterfaceClient JSON error: " + std::string(e.what()));
    }
}
...
std::future<Enum1Enum> SameEnum1InterfaceClient::func1Async(Enum1Enum param1, std::function<void(Enum1Enum)> callback)
{
    if(m_client == nullptr) {
