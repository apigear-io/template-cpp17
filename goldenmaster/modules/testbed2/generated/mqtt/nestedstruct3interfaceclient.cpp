...
std::shared_ptr<NestedStruct3InterfaceClient> NestedStruct3InterfaceClient::create(std::shared_ptr<ApiGear::MQTT::Client> client)
{
    return std::make_shared<NestedStruct3InterfaceClient>(client);
}
...
void NestedStruct3InterfaceClient::setProp1Local(const std::string& args)
{
    try {
        ...
        AG_LOG_ERROR("NestedStruct3InterfaceClient JSON error: " + std::string(e.what()));
    }
}
...
std::future<NestedStruct1> NestedStruct3InterfaceClient::func3Async(const NestedStruct1& param1, const NestedStruct2& param2, const NestedStruct3& param3, std::function<void(NestedStruct1)> callback)
{
    ...
    return std::async(std::launch::async, [this, callback,
                    ...
                    param3]()
        {
            ...
            auto responseId = registerResponseHandler(responseHandler);
            m_client->invokeRemote(topic, responseTopic, nlohmann::json::array({param1, param2, param3}).dump(), responseId);
            return resultPromise->get_future().get();
        }
    );
}
