...
TEST_CASE("Nats  testbed2 NestedStruct3Interface tests")
{
    ...
    serviceNestedStruct3Interface->_subscribeForIsReady([&is_serviceReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
    ...
    if (serviceNestedStruct3Interface->_is_ready())
    {
        is_serviceReady = true;
    }
    lock.lock();
    ...
    clientNestedStruct3Interface->_subscribeForIsReady([&is_clientReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
                is_clientReady = true;
    ...
    if (clientNestedStruct3Interface->_is_ready())
    {
        is_clientReady = true;
    }
    lock.lock();
