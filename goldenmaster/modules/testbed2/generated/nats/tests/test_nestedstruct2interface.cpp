...
TEST_CASE("Nats  testbed2 NestedStruct2Interface tests")
{
    ...
    {
        [[maybe_unused]] auto result = clientNestedStruct2Interface->func1(NestedStruct1());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func1 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientNestedStruct2Interface->func1Async(NestedStruct1());
    ...
    {
        [[maybe_unused]] auto result = clientNestedStruct2Interface->func2(NestedStruct1(), NestedStruct2());
        // CHECK EFFECTS OF YOUR METHOD HERE
    }
    SECTION("Test method func2 async")
    {
        std::atomic<bool> finished = false;
        auto resultFuture = clientNestedStruct2Interface->func2Async(NestedStruct1(), NestedStruct2());
        auto f = std::async(std::launch::async, [&finished, &resultFuture, &m_wait]() {resultFuture.wait(); finished = true; m_wait.notify_all();});
        lock.lock();
        REQUIRE( m_wait.wait_for(lock, std::chrono::milliseconds(timeout), [&finished](){ return finished == true; }));
        lock.unlock();
