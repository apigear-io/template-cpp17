...
TEST_CASE("Nats  testbed2 NestedStruct2Interface tests")
{
    ...
    serviceNestedStruct2Interface->_subscribeForIsReady([&is_serviceReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
    ...
    if (serviceNestedStruct2Interface->_is_ready())
    {
        is_serviceReady = true;
    }
    lock.lock();
    ...
    clientNestedStruct2Interface->_subscribeForIsReady([&is_clientReady, &m_wait](auto is_ready)
        {
            if (is_ready)
            {
                is_clientReady = true;
    ...
    if (clientNestedStruct2Interface->_is_ready())
    {
        is_clientReady = true;
    }
    lock.lock();
